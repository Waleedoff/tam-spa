...
function TextAreaStlye({
  ...
  return (
    <div dir={props.dir} className="relative flex flex-col">
      <div
      ...
      <textarea
        {...props}
        rows={rows}
        className={`${getInputVarient()} peer mt-1 w-full resize-none border-gray-300 border-opacity-50 text-sm transition hover:border-gray-400 focus:border-blue-500 focus:outline-none disabled:bg-white`}
      />
      <label
        className={`${getLabelVarient()} absolute px-1 text-sm text-gray-500 text-opacity-80 disabled:!bg-transparent disabled:bg-none peer-focus:text-blue-500`}
      >
        {t(`${label}`)}
      </label>
