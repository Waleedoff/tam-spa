  ...
  return (
    ...
    >
      ...
      <input
        {...props}
        type="text"
        className={`peer h-14 border-gray-300 text-sm transition focus:border-blue-500 focus:outline-none disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-500 disabled:shadow-none ${getInputVarient()}`}
      />
      <label
        className={`absolute bottom-1/2 text-gray-600 transition duration-200 disabled:!bg-transparent peer-focus:text-blue-500 ${getLabelVarient()} disabled:bg-none`}
      >
        {t(`${label}`)}
      </label>
      <div
